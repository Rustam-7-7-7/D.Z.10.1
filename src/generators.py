def filter_by_currency(transactions, currency):
    ...
    for transaction in transactions:
        try:
            # Проверяем, есть ли в транзакции валюта и совпадает ли её код с currency_code
...
def card_number_generator(start, end):
    ''' Генератор, который выдает номера банковских карт в формате XXXX XXXX XXXX XXXX, где
         X — цифра номера карты. Генератор может сгенерировать номера карт в заданном диапазоне
          от 0000 0000 0000 0001 до 9999 9999 9999 9999. Генератор должен принимать начальное и
